...
class MoveImages:
    ...
    def move_images(self, path_from: Path, path_to: Path):
        ...
        for dir in tqdm(self.__array_dirs,desc="Move files"):
            self.__move_images_to_folder(path_from, path_to, dir, self.__array_images[:validation_idx])
            self.__move_images_to_folder(path_from, path_to, dir, self.__array_images[validation_idx: test_idx])
            self.__move_images_to_folder(path_from, path_to, dir, self.__array_images[test_idx:])